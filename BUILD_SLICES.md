...
**Implementation notes (done 2026-07-31 — local half; VPS deploy prepared, not yet executed):**

All code-side work for this slice is built, verified locally, and pushed. **The deploy itself is handed off, per the user's own explicit request this session** — no SSH from a session to the VPS, so `DEPLOYMENT.md` §8 carries the exact command sequence and is the actual deliverable for the deploy half; this repo's own "Done when" line stays open until that runs and the live smoke test (below) passes.
