...
- **`FoodThread`** *(added PC-1, 2026-08-19)* — `sellerId`, `clientId`, `clientEmail?` (snapshot, same identity-lookup gap as `FoodOrder.clientEmail`), `lastMessageAt?`, `createdAt`. `@@unique([sellerId, clientId])` — one conversation per relationship, spanning every order the pair ever transacts.
- **`FoodMessage`** *(was `FoodOrderMessage`)* — `threadId`, `orderId?`, `senderUserId`, `originalText`, `originalLocale`, `translations Json` (locale → text, computed once via kap64-translate), `attachmentPath?` + `attachmentKind?` (`PHOTO`), `createdAt`, `readAt?`.
  ~~One thread per order, no separate thread entity needed in MVP.~~ **Superseded 2026-08-19** by the user ruling recorded in `Apoyo-Demia/PRE_LAUNCH_CHECKLIST.md` §5: conversation died with the order and there was no seller inbox at all, so a buyer writing months later had nowhere for it to land. The thread now owns the message (`onDelete: Cascade`) and the order is optional context on it (`onDelete: SetNull`).
  ⚠ **The order is still the gate, just not the container.** Creating a usable thread requires at least one order the seller ENGAGED with (`lib/thread.ts`'s `ENGAGED_ORDER_STATUSES` — accepted/completed/declined/cancelled, deliberately NOT pending or expired, which a stranger can produce unilaterally). Order-scoping used to make unsolicited contact structurally impossible; that protection is now explicit application logic and must not be weakened to "a thread row exists".
- **`FoodSeller` conversation controls** *(added PC-1)* — `postOrderMessaging` (opt OUT of chat between orders; ⚠ **defaults to true**, it is an escape hatch, not a feature to discover), `messageReadReceipts` (whether the buyer is shown "Read"; disclosure only, `readAt` is written regardless), `notificationPrefs Json` (per-category delivery, only `chat` read today — order-lifecycle mail is deliberately not mutable).
...
- **Thread:** ~~`FoodOrderMessage` per order~~ → **`FoodMessage` per `FoodThread`, one thread per (seller, buyer) pair** (PC-1, 2026-08-19 — see Part D). Translations stored on the message at send time (kap64-translate, `lib/translate.ts` pattern); photo attachments (e.g. cake-design references). Transport: fetch-on-load + light polling on the open order page AND on the Messages section (MVP) → ws upgrade Phase 9, same table.
- **Retention (PC-1):** the order's `onDelete: Cascade` used to be the only cleanup this data had, and lifting conversation off the order removed it. Replaced by `sweepIdleThreads` on the existing `food-sweep` runner: a thread is deleted after 12 months with no message, ⚠ **unless the pair still has an ACTIVE order** — a booking placed a year out (wedding cake, Christmas catering) must not have its arrangements swept away for being quiet. ⚠ **"Active" is `orderIsActive`, not a status test**, because `ACCEPTED` has no automatic exit in this app (only the seller marks `COMPLETED`): a PENDING request inside its `respondBy` window, or an ACCEPTED booking whose `fulfillmentAt` + 30 days hasn't passed. Without that, an order abandoned years ago would shield its conversation forever. ⚠ It deliberately does not CLOSE such an order — auto-expiring a stale `ACCEPTED` order is a real but separate order-lifecycle gap, not a cleanup job's decision.
