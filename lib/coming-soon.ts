...
export type FeaturePhase = 2 | 3 | 4;
 ...
 */
export type FeatureIcon = "utensils" | "message" | "receipt" | "user" | "chefHat";
 ...
 * - `becomeSeller`  — a seller-facing action reachable from a buyer surface,
 *   which the brief asks to register. It is stubbed rather than linked because
 *   FOOD's `vertical_registration_config` row is seeded `false` (Slice 3), on
 *   purpose: onboarding does not exist until Slice 13, and a CTA leading to a
 *   disabled registration is worse than no CTA.
...
export const COMING_SOON_FEATURES = {
  becomeSeller: { phase: 2, slice: 13, icon: "chefHat" },
