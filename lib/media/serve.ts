...
/**
 * The local-disk driver's read side, shared by BOTH the buyer-reachable
 * `/api/media/[...path]` route and the seller-reachable
 * `/api/food/media/[...path]` route (ecosystem ruling E14) — one handler, two
 * routes, so the two surfaces can never drift.
 *
...
export async function serveMedia(segments: string[]): Promise<NextResponse> {
