...
/**
 * `FoodListingPhoto` upload — `app/api/seller/media`'s shape, one relation hop
 * further out (Slice 13's rationale for that route applies unchanged: ingest
 * and persist in ONE ownership-checked request, so the browser never handles a
 * storage key it could substitute for one it doesn't own).
 *
 ...
 *
 * Rate-limited under the SAME buckets as `/api/seller/media` (`seller-media:*`)
